...
CREATE TABLE USERS (
  ...
  USER_NAME VARCHAR(100) CONSTRAINT USERS_NN_USERNAME,
  USER_EMAIL EMAIL,
...
CREATE TABLE COMMENTS (
  INFO_ID BIGINT GENERATED ALWAYS AS IDENTITY CONSTRAINT COMMENTS_PK_INFOID PRIMARY KEY,
  ...
  CONSTRAINT COMMENTS_PRODUCTS_FK_PRODUCTID FOREIGN KEY (PRODUCT_ID) REFERENCES USERS (PRODUCT_ID) ON DELETE CASCADE
);
...
CREATE TABLE ORDERS (
  ...
  ORDER_QUANTITY INTEGER CONSTRAINT PRODUCTS_CK_QTY CHECK (PRODUCT_QUANTITY >= 0 AND PRODUCT_QUANTITY IS NOT NULL),
...
CREATE OR REPLACE FUNCTION SEEDING(
  ...
  PRODUCT_SKU_IN TEXT,
  PRODUCT_IMAGE_IN TEXT,
  PRODUCT_PRICE_IN DECIMAL,
)
RETURNS VOID
LANGUAGE 'plpgsql'
AS $$
DECLARE
  CAT_ID BIGINT;
BEGIN
  CAT_ID = INSERT INTO CATEGORIES (CATEGORY_NAME)
           VALUES (CATEGORY_NAME_IN)
           RETURNING CATEGORY_ID;
  INSERT INTO PRODUCTS
  (PRODUCT_NAME, PRODUCT_DESCRIPTION, PRODUCT_SKU, PRODUCT_IMAGE, PRODUCT_PRICE, CATEGORY_ID)
  VALUES
  (PRODUCT_IMAGE_IN, PRODUCT_DESCRIPTION_IN, PRODUCT_SKU_IN, PRODUCT_IMAGE_IN, PRODUCT_PRICE_IN, CAT_ID);
END;
$$;
